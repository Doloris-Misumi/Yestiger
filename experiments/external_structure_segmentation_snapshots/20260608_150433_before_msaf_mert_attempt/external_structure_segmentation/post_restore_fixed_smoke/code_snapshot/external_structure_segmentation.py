...
def cbm_grid(fill_to_count: bool) -> List[Dict[str, Any]]:
    candidates = []
    feature_sets = ("full",) if fill_to_count else ("timbre_chroma", "full")
    min_sizes = (4, 6) if fill_to_count else (4, 6, 8)
    max_sizes = (12, 16) if fill_to_count else (12, 16, 20, 24)
    penalty_weights = (0.04, 0.08) if fill_to_count else (0.04, 0.08, 0.12)
    smooth_sigmas = (0.0, 0.5)
...
def tuned_wrappers(args: argparse.Namespace) -> List[Dict[str, Any]]:
    ...
    wanted = set(args.wrapper or ["all"])
    if "all" not in wanted:
        wrappers = [wrapper for wrapper in wrappers if str(wrapper["key"]) in wanted]
    return wrappers
...
def main() -> int:
    ...
    parser.add_argument("--skip-fixed", action="store_true", help="Skip fixed candidates and run only selected LOSO-tuned wrappers.")
    parser.add_argument(
        "--wrapper",
        action="append",
        choices=["all", "cbm", "cbm_fill", "foote", "agglomerative", "spectral"],
        help="LOSO-tuned method family to run. Can be repeated.",
    )
