...
> 在 leave-one-song-out 设置下，加入音频信号特征后，macro-F1 从 0.457 提升到 0.485。这说明音频特征确实提供了粗粒度结构标签之外的信息。
...
> 我们不只在小节级分类，也把连续相同 role 的小节合并成 call spans，再和人工 call spans 比较。加入音频特征后的 LOSO audio RF 在合并段上的 time-weighted role accuracy、macro role IoU 和 boundary F1 都是当前最高。
