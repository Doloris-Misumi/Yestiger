...
For merged spans, the audio-feature soft-voting model performs best on time-weighted role accuracy and macro role IoU. The previous audio RF keeps a slightly higher boundary F1, so a future boundary-specific decoder is still a useful next step.
