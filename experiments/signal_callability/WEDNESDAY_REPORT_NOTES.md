...
| audio RF + boundary top-k decoder | 0.504 | 0.479 |
...
| LOSO audio RF + boundary top-k | 0.497 | 0.311 | 0.502 |

讲法：

> 我们不只在小节级分类，也把连续相同 role 的小节合并成 call spans，再和人工 call spans 比较。优化后的 soft-voting 模型在 time-weighted role accuracy 和 macro role IoU 上最高；专门的 boundary top-k decoder 把 Boundary F1 从 0.479 提升到 0.502，是当前边界分割最好的方法。
...
experiments/signal_callability/poppindream/poppindream.merged.loso_audio_rf_boundary_topk_struct_heavy_len.callbook.md
experiments/signal_callability/poppindream/poppindream.merged.loso_audio_rf_boundary_topk_struct_heavy_len.call_spans.json
```
...
| manual fine | allin1 + learned signal | 0.774 | 0.504 | 0.610 | 244 | 159 |
| manual coarse | allin1 structure | 0.843 | 0.572 | 0.682 | 159 | 108 |
| manual coarse | fused novelty top-k | 0.289 | 0.289 | 0.289 | 159 | 159 |
| manual coarse | best searched allin1 + signal | 0.829 | 0.579 | 0.681 | 159 | 111 |

讲法：

> 因为我的人工标注比 allin1 多了很多细段落类型，所以直接拿全部人工边界比会惩罚 allin1。这里做了两套目标：`manual fine` 保留所有人工段落边界；`manual coarse` 先把 verse/pre-chorus/chant、chorus/post-chorus、inst/interlude 等细标签合并成 allin1 更接近的粗类别，再做边界比较。搜索结果说明：粗结构上 allin1 仍然最强，当前信号方法没有稳定超过它；但在 fine target 上，`allin1 + learned signal` 可以把 F1 从 0.545 提升到 0.610，说明音频信号更适合做 allin1 后的细粒度边界补充，而不是直接替代 allin1。
...
### Q8: 如果老师重点问“分得准不准”，应该讲哪个结果？

回答：

> 讲 `audio RF + boundary top-k decoder`。它不是简单看每个小节 role 是否改变，而是把候选边界按照 allin1 结构边界、fused novelty、onset novelty、energy novelty 和相邻片段长度打分，只保留训练集估计数量附近的高分边界。这样预测边界数从 394 降到 325，更接近人工的 316，Boundary F1 从 0.479 提升到 0.502。
