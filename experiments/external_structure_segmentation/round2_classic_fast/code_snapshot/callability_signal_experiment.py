...
def indexed_role_spans(roles: Sequence[str]) -> List[Dict[str, Any]]:
    spans = []
    if not roles:
        return spans
    start = 0
...
def target_boundary_rate_from_grouped_rows(grouped: Dict[str, List[Dict[str, Any]]], test_song: str) -> float:
    total = 0
    boundaries = 0
    for song_id, song_rows in grouped.items():
        if song_id == test_song:
            continue
        roles = [normalize_role(row["target"]["call_role"]) for row in song_rows]
        if len(roles) <= 1:
            continue
        total += len(roles) - 1
        boundaries += sum(1 for prev, current in zip(roles, roles[1:]) if prev != current)
    return boundaries / total if total else 0.0


def majority_role_from_sequence(roles: Sequence[str]) -> str:
    if not roles:
        return "keepspace"
    return Counter(normalize_role(role) for role in roles).most_common(1)[0][0]


def boundary_topk_score(row: Dict[str, Any], prev_len: int, next_len: int) -> float:
    features = row.get("features") or {}
    novelty = row.get("novelty") or {}
...
def roles_from_selected_boundaries(base_roles: Sequence[str], selected_boundaries: Sequence[int]) -> List[str]:
    n = len(base_roles)
    boundaries = [0] + sorted({int(index) for index in selected_boundaries if 0 < int(index) < n}) + [n]
    decoded: List[str] = []
    previous_role = None
    for left, right in zip(boundaries[:-1], boundaries[1:]):
        role = majority_role_from_sequence(base_roles[left:right])
...
def decode_boundary_topk_roles(
    grouped_rows: Dict[str, List[Dict[str, Any]]],
    song_id: str,
    song_rows: Sequence[Dict[str, Any]],
    base_roles: Sequence[str],
    rate_scale: float = 1.10,
) -> List[str]:
    spans = indexed_role_spans(base_roles)
    if len(spans) <= 1:
        return [normalize_role(role) for role in base_roles]
    rate = target_boundary_rate_from_grouped_rows(grouped_rows, test_song=song_id)
    target_count = int(round(rate * max(0, len(song_rows) - 1) * rate_scale))
    candidates = []
    for span_index in range(1, len(spans)):
        ...
        candidates.append((index, boundary_topk_score(song_rows[index], prev_len, next_len)))
    selected = sorted(candidates, key=lambda item: item[1], reverse=True)[: min(target_count, len(candidates))]
    return roles_from_selected_boundaries(base_roles, [index for index, _ in selected])


def add_boundary_topk_predictions(
    rows: Sequence[Dict[str, Any]],
    base_predictions: Sequence[Dict[str, Any]],
    method: str = "loso_audio_rf_boundary_topk_struct_heavy_len",
) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
    grouped: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
    for row in rows:
        grouped[str(row["_song_id"])].append(row)
    ...
    for song_id, song_rows in grouped.items():
        base_roles = [
            normalize_role(base_map.get((song_id, int(row["bar_index"])), "keepspace"))
            for row in song_rows
        ]
        decoded_roles = decode_boundary_topk_roles(grouped, song_id, song_rows, base_roles)
        target_roles = [normalize_role(row["target"]["call_role"]) for row in song_rows]
        y_true_all.extend(target_roles)
        y_pred_all.extend(decoded_roles)
        for row, target, pred in zip(song_rows, target_roles, decoded_roles):
            predictions.append(
                {
                    "song_id": song_id,
                    "bar_index": row["bar_index"],
                    ...
                    "prediction": pred,
                    "method": method,
                    "feature_set": "audio_rf_boundary_topk",
                    "model_kind": "postprocess",
                    "decoder": "topk_struct_heavy_len",
                }
            )
    return classification_metrics(y_true_all, y_pred_all), predictions
...
def run_loso_experiments(results: Sequence[Dict[str, Any]], out_dir: Path) -> Dict[str, Any]:
    ...
    boundary_method = "loso_audio_rf_boundary_topk_struct_heavy_len"
    if "loso_audio_rf" in predictions_by_method:
        boundary_metrics, boundary_predictions = add_boundary_topk_predictions(
            rows,
            predictions_by_method["loso_audio_rf"],
            method=boundary_method,
        )
        metrics_by_method[boundary_method] = boundary_metrics
        predictions_by_method[boundary_method] = boundary_predictions
        all_predictions.extend(boundary_predictions)
